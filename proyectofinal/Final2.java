...
class Estudiante {
    ...
    public int getHoDeporte() { return HoDeporte; }
    public int getHoCultura() { return HoCultura; }
    public int getHoSaludformaind() { return HoSaludformaind; }

    public void acumulaciondeHoras(String mismacategoria, int Ho) {
        if (mismacategoria.equalsIgnoreCase("Deporte")) HoDeporte += Ho;
        if (mismacategoria.equalsIgnoreCase("Cultura")) HoCultura += Ho;
        if (mismacategoria.equalsIgnoreCase("Salud")) HoSaludformaind += Ho;
    }

    public int getActividades() { return Actividades; }
    public void añadir() { this.Actividades++; }
    public void reiniciar() { this.Actividades = 0; }
    public int getTotalHo() { return HoDeporte + HoCultura + HoSaludformaind; }
}
...
class Actividades {
    ...
    public int getpartMaximo() { return partMaximo; }
...
class Inscripciones {
    ...
    public Actividades getActividades() { return actividades; }
}
...
class gestioncupos {
    ...
    public void inscripcionEstudiante(Estudiante estudiante, Actividades actividades) {
        ...
        if (actividades.isAltoImpacto() && estudiante.getpromedio() < 4.0) {
            System.out.println("  Lo sentimos solicitud rechazada: Promedio insuficiente.");
            return;
        }
        
        
        for (Inscripciones ins : inscripcion) {
            if (ins.getEstudiante() == estudiante && ins.getActividades().getHorario() == actividades.getHorario()) {
                System.out.println("  Lo sentimos: Cruce de horarios con " + ins.getActividades().getNombre());
                return;
            }
        }
        ...
        int cuposLlenos = 0;
        for (Inscripciones ins : inscripcion) {
            if (ins.getActividades() == actividades) {
                cuposLlenos++;
            }
        }
        ...
        if (cuposLlenos < actividades.getpartMaximo()) { 
            ...
            System.out.println("  Atencion: cupos llenos. A lista de espera.");
        }
    }
    ...
    public void cancelarIncripsiones(Estudiante estudiante, Actividades actividades, boolean conAntelacion) {
        ...
        if (encontrada != null) {
            ...
            if (!conAntelacion) {
                // CORRECCIÓN: Ahora penaliza según la categoría de la actividad cancelada, no solo Deporte
                estudiante.acumulaciondeHoras(actividades.getCategoria(), -2);
                System.out.println("  AVISO: Penalizacion de -2 horas.");
            }

            // CORRECCIÓN: Pasamos al siguiente por el método oficial para que valide sus datos
            if (!listadeEspera.isEmpty()) {
                Estudiante siguiente = listadeEspera.remove(0);
                inscripcionEstudiante(siguiente, actividades);
            }
        }
    }
    ...
    public void completarActividad(Estudiante est, Actividades act, int horas) {
        ...
        if (est.getActividades() == 3) {
            ...
            System.out.println("\n¡BONO!: " + est.getNombre() + " +5 horas en " + act.getCategoria());
            est.reiniciar();
        }
    }
    ...
    public void ReporteFinal(Estudiante est) {
        ...
        System.out.println("  Horas Salud: " + est.getHoSaludformaind() + "/10");
        ...
        if (est.getHoDeporte() >= 10 && est.getHoCultura() >= 10 && est.getHoSaludformaind() >= 10) {
            System.out.println("  Verificación: Cumple requisitos.");
        } else {
            System.out.println("  Verificación: No habilitado");
        }
    }
}
